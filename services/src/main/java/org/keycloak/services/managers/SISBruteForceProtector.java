/*
 * provide sis brute force protection functionalities
 */
 ...
 * @version $Revision: 1 $
 */
public class SISBruteForceProtector implements Runnable, BruteForceProtector {
    private static final Logger logger = Logger.getLogger(SISBruteForceProtector.class);
    ...
    public SISBruteForceProtector(KeycloakSessionFactory factory) {
        this.factory = factory;
    }

    public void failure(KeycloakSession session, LoginEvent event) {
        logger.info("SISBruteForceProtector:failure - failure");
...
//                user.setSingleAttribute(DISABLED_REASON, DISABLED_BY_PERMANENT_LOCKOUT);
            ...
            if (last > 0 && deltaTime < realm.getQuickLoginCheckMilliSeconds()) {// this is the place where temporarily locking happen
    ...
    private void success(KeycloakSession session, LoginEvent event) {
        ...
        if (logger.isDebugEnabled()) {
            ...
            logger.infov("user {0} successfully logged in, clearing all failures", model.getUsername());
        }
        user.clearFailures();
    }
